...
#                           --task ceval --split validation --lang vi --n_shot 5 --batch_size 4 --save_name result
...
@dataclass # định dạng dữ liệu đầu vào theo template ngôn ngữ (vi, en)
class EvalTemplate: 
    ...
    def parse_example(  # Chuyển một mẫu (dạng dict) thành chuỗi câu hỏi với các lựa chọn và đáp án.
        self,
    ...
    def format_example(     # Tạo prompt theo kiểu n-shot (thêm lịch sử câu hỏi trước đó)
        self,
...
eval_templates = {
    ...
    "vi": EvalTemplate(
        system="Sau đây là các câu hỏi trắc nghiệm (có đáp án) về {subject}.\n\n",
        choice="\n{choice}. {content}",
        answer="\nCâu trả lời: ",
        prefix=" "
    )
...
def batch_inference(    # Trích xuất logits từ mô hình, chọn token cuối đại diện cho lựa chọn (A/B/C/D), chọn đáp án có xác suất cao nhất từ softmax.
...
def evaluate(
    ...
    lang: Optional[Literal["en", "vi"]] = "en",
    ...
    # Lặp qua từng môn học (subject), tạo prompt theo n-shot, encode input
    ...
    for subject in pbar:
        ...
        # Gửi batch vào mô hình để lấy kết quả.
        ...
        # Tính toán accuracy theo từng category và trung bình
